...
%------------%
  ...
  \headingBf{Renesas Design Vietnam}{08/2022 -- 05/2024}
   ...
   \headingBf{ClinicPRO: Clinic Management App (Freelance)}{11/2025 -- Present}
   \begin{resume_list}
     \item \textbf{Description}: A comprehensive management application for clinics with OCR, interactive charts, and web support.
     \item \textbf{Role}: Full Stack Developer (Solo)
     \item \textbf{Key Contributions}:
        \begin{itemize}[itemsep=0pt, leftmargin=10pt, label=$\circ$]
           \item Implemented smart record importing using \textbf{PaddleOCR} with spatial parsing.
           \item Built automated backup system with daily rotation and cloud sync.
           \item Engineered an interactive dental charting UI and led the transition to a \textbf{Flask-based} web architecture.
        \end{itemize}
     \item \textbf{Achievement}: Deployed a complete solution handling real-world clinic operations.
     \item \textbf{Tech Stack}: Python, Flask, CustomTkinter, PaddleOCR, SQLite, ReportLab.
   \end{resume_list}
