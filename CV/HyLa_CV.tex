  ...
  \section{\faBriefcase\ WORK EXPERIENCE}
   ...
   \begin{resume_list}
     ...
     \item \textbf{\faVideo\ Project: High-Performance IP Camera (IP\_Cam)}
       \begin{itemize}[itemsep=1pt, leftmargin=15pt, label=-]
         \item \textbf{Description}: A high-performance audio/video pipeline featuring low-latency Opus audio integration and modular build configuration.
         \item \textbf{Role}: Core System Engineer
         \item \textbf{Key Contributions}:
           \begin{itemize}[itemsep=0pt, leftmargin=10pt, label=$\circ$]
              \item \textbf{Day/Night Mode (DNM)}: Developed the core HAL and State Machine to manage ISP/IR-Cut transitions. Ensured hardware configurations (Sensor registers, GPIOs) are set correctly and services run smoothly during mode switches.
              \item Integrated \textbf{Opus} codec and restored the full Resampler $\rightarrow$ Mixer $\rightarrow$ Playback pipeline.
              \item Refined \textbf{Buildroot} and \textbf{CMake} configurations for modular feature toggling.
           \end{itemize}
         \item \textbf{Achievement}: Achieved lip-sync accuracy with low-latency audio; Delivered robust Day/Night transitions.
         \item \textbf{Tech Stack}: C, Buildroot, Opus, CMake, ALSA, HAL.
       \end{itemize}
